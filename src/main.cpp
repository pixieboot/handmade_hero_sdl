...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
  // init the lib, here we make a window so we only need video capabilities
  ...
  // create a window
  SDL_Window* window = SDL_CreateWindow("text", windowStartWidth, windowStartHeight, SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIGH_PIXEL_DENSITY);
  if(not window) {
    return SDL_Fail();
  }
  ...
  SDL_FRect text_rect {
    ...
    .h = float(SDL_GetNumberProperty(messageTexProps, SDL_PROP_TEXTURE_WIDTH_NUMBER, 0))
  };
